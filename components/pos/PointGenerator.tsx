...
export default function PointGenerator({ onClose }: { onClose?: () => void }) {
  ...
  const qrUrl = token 
    ? `https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${encodeURIComponent(`https://liff.line.me/${process.env.NEXT_PUBLIC_LIFF_ID || '2009322178-2dtfXAvi'}/member?claimToken=${token}`)}`
    : null;
