...
import { QrCode, RefreshCcw, X, Edit2, CheckCircle2, ArrowLeft } from 'lucide-react';
...
export default function PointGenerator({ onClose }: { onClose?: () => void }) {
  ...
  const pointsToGenerate = amount || 0;
  const presets = [10, 20, 50, 100];

  const generateQR = async () => {
    if (!pointsToGenerate || pointsToGenerate <= 0) {
    ...
    <div className="bg-[#f5f5f7] rounded-[32px] shadow-2xl w-full max-w-[420px] mx-auto flex flex-col overflow-hidden font-sans border border-white/50">
      ...
      <div className="flex items-center justify-between px-6 pt-6 pb-2">
        ...
        <h2 className="text-[17px] font-semibold text-gray-900 tracking-tight">
          {token ? (locale === 'en' ? 'Reward Ready' : 'คิวอาร์โค้ดพร้อมแล้ว') : (locale === 'en' ? 'Generate Reward Points' : 'แจกคะแนนสะสม')}
        </h2>
          ...
          {!token ? (
            ...
            >
              <p className="text-[11px] font-medium text-gray-400 tracking-widest uppercase mb-4">
                {locale === 'en' ? 'Points to award' : 'จำนวนคะแนนที่จะให้'}
              </p>
              ...
              <div className="flex items-start justify-center h-[90px] mb-8">
                ...
                <span className="text-xl font-semibold text-gray-400 mt-3 ml-1">
                  PTS
                </span>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 mb-10 w-full max-w-[320px]">
                {presets.map((val) => (
                  ...
                  >
                    {val} <span className="text-[11px] opacity-70 ml-0.5">PTS</span>
                  </button>
                ))}
              ...
              <button 
                onClick={generateQR}
                disabled={loading || !pointsToGenerate}
              ...
              <p className="text-[12px] text-gray-400 mt-4 text-center">
                {locale === 'en' ? 'This code awards points to the customer.' : 'โค้ดนี้ใช้สำหรับให้ลูกค้าสแกนเพื่อรับคะแนน'}
              </p>
            </motion.div>
          ) : (
            <motion.div 
            ...
            >
              ...
              {/* QR Code Container */}
              <div className="bg-white p-6 rounded-[32px] shadow-sm mb-6 relative w-[240px] h-[240px] flex items-center justify-center border border-gray-100">
              ...
              <div className="text-center mb-8">
                 ...
                 <p className="text-[40px] font-medium text-gray-900 leading-none tracking-tight">
                   {pointsToGenerate} <span className="text-[16px] text-gray-400">PTS</span>
                 </p>
                 <p className="text-[11px] text-gray-400 font-medium tracking-widest mt-2 uppercase">
              ...
              >
                <ArrowLeft size={18} /> {locale === 'en' ? 'Generate Another' : 'สร้างใหม่อีกครั้ง'}
              </button>
            </motion.div>
          )}
        </AnimatePresence>
