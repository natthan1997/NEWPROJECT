                     ...
                     {filteredItems.map(item => {
                         ...
                         <div key={item.id} className={`group bg-white border flex flex-col transition-all overflow-hidden rounded-2xl ${!currentStatus ? 'border-red-200 bg-red-50/30 opacity-80' : 'border-[#E5E5DF] hover:shadow-2xl hover:-translate-y-1'}`}>
                             <div className="aspect-[4/3] bg-gray-50 overflow-hidden relative">
                                 {item.image_url ? <img crossOrigin="anonymous"  src={item.image_url ? `${item.image_url}?v=8` : ''} className={`w-full h-full object-cover transition-transform duration-700 ${!currentStatus ? 'grayscale' : 'group-hover:scale-105'}`} /> : <div className="w-full h-full flex items-center justify-center"><ImageIcon size={32} className="text-gray-200" /></div>}
                                 {!currentStatus && (
                   ...
                   {filteredItems.map(item => (
                       <div key={item.id} className="group relative flex flex-col bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer" onClick={() => { setEditingItem(item); fetchItemLinks(item.id); setIsEditorOpen(true); }}>
                           <div className="aspect-square relative overflow-hidden bg-gray-50">
                               {item.image_url ? (
                                   <img crossOrigin="anonymous"  src={item.image_url ? `${item.image_url}?v=8` : ''} alt={item.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                               ) : (
                          ...
                          {itemsInCat.map((item, idx) => (
                            ...
                            >
                              ...
                              <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-xl border border-[#F0F0E8] bg-gray-50">
                                {item.image_url ? <img crossOrigin="anonymous"  src={item.image_url ? `${item.image_url}?v=8` : ''} className="h-full w-full object-cover" /> : <ImageIcon size={16} className="text-gray-300" />}
                              </div>
                                                ...
                                                <tr key={item.id} className="group hover:bg-gray-50/40 transition-colors align-middle">
                                                    ...
                                                    <td className="p-4 text-center">
                                                        <div className="relative w-12 h-12 rounded-xl bg-gray-100 overflow-hidden mx-auto shadow-sm group-hover:shadow transition-all">
                                                            {item.image_url ? (
                                                                <img crossOrigin="anonymous"  src={item.image_url ? `${item.image_url}?v=8` : ''} className="w-full h-full object-cover" />
                                                            ) : (
                          ...
                          <div className="flex flex-col gap-4">
                              <div className="aspect-video bg-gray-50 border border-dashed border-gray-200 flex items-center justify-center relative overflow-hidden group">
                                  {editingItem.image_url ? (
                                      <>
                                          <img crossOrigin="anonymous"  src={editingItem.image_url ? `${editingItem.image_url}?v=8` : ''} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                                          <button 
