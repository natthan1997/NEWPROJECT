...
import { 
  ChevronLeft, History, Gift, TrendingUp, User, Info, X, Check, ArrowRight
} from 'lucide-react';
          ...
          <div className={\`w-full rounded-2xl p-6 \${currentTier.bg} transition-colors duration-500\`}>
            <div className="flex items-center gap-4 mb-8">
              ...
              <div className="ml-auto text-right">
                <span className={\`text-[13px] font-bold uppercase tracking-wider \${currentTier.text}\`}>
                  {currentTier.name}
                </span>
              </div>
            </div>
                ...
                <div>
                  ...
                  <div className="h-1 w-full bg-black/5 rounded-full overflow-hidden">
                    <motion.div 
                      ...
                      className={\`h-full rounded-full \${currentTier.text.replace('text-', 'bg-')}\`}
                    />
                  </div>
                  <p className="text-[12px] text-gray-500 mt-2 text-center">
                    อีก <span className="font-medium text-gray-900">{(nextTier.minPoints - totalAccumulated).toLocaleString()}</span> {dict.pointsToNextTier} <span className="font-medium text-gray-900">{nextTier.name}</span>
                  </p>
                </div>
              ) : (
...
console.log('Redesigned LiffMemberPage V5 (Ultra Minimal / Muji Style)');
