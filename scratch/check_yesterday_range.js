...
async function checkYesterdayRange() {
  const targetBranchId = '1f3fc496-d89e-4323-a66e-4fcd555444e9';
  // Date range up to July 24, 2026
  ...
  const totalRevenue = branchOrders.reduce((sum, o) => sum + Number(o.total_amount || 0), 0);
  const totalDiscount = branchOrders.reduce((sum, o) => sum + Number(o.discount_amount || 0), 0);

  console.log(`=== UP TO YESTERDAY (JULY 24) ===`);
  console.log(`Orders count: ${branchOrders.length}`);
  console.log(`Gross Revenue: ${totalRevenue.toFixed(2)} THB`);
  console.log(`Discount: ${totalDiscount.toFixed(2)} THB`);
  ...
  if (orderIds.length > 0) {
    ...
    items.forEach(item => {
      ...
      yesterdayCogs += finalUnitCost * (item.quantity || 1);
    });
  }

  console.log(`Calculated COGS up to July 24: ${yesterdayCogs.toFixed(3)} THB`);
}
