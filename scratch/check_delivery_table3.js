...
async function main() {
  const { data: latestLiffOrder } = await supabase.from('pos_orders')
    .select('id, order_number, net_total, delivery_fee, order_source, points_earned, status, created_at')
    .eq('order_source', 'liff')
    .order('created_at', { ascending: false })
    .limit(3);
  console.log("Recent LIFF Orders:");
  ...
  console.log("History for recent LIFF orders:");
  console.log(hist);
}
