...
if (!shopStatusJSX) throw new Error("Failed to extract shopStatusJSX");
...
// Replace empty operational block
posContent = posContent.replace(
    /\{activeTab === 'operational' && \([\s\S]*?<\/div>\s*\)\}/,
...
// Replace empty delivery block
posContent = posContent.replace(
    /\{activeTab === 'delivery' && \([\s\S]*?<\/div>\s*\)\}/,
    `{activeTab === 'delivery' && (
                            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                ${deliveryRulesJSX}
                                ${deliveryGPJSX}
...
console.log('POSShopSettings.tsx repaired successfully.');
