  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] text-[#1A1A18] font-sans pb-24">
      ...
      <main className="px-5 pt-6 relative z-10 max-w-lg mx-auto flex flex-col gap-8">
        ...
        >
          <div className={`p-7 rounded-[32px] relative flex flex-col overflow-hidden shadow-2xl ${currentTier.cardBg || 'bg-[#1A1A18]'}`}>
            ...
            <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
                ...
                <svg className="absolute bottom-0 right-0 opacity-20 w-48 h-48 translate-x-1/4 translate-y-1/4" viewBox="0 0 24 24" fill="white" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="relative z-10 text-white">
                <div className="flex justify-between items-start mb-10">
                    <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-full overflow-hidden bg-white/20 border-2 border-white/40 shrink-0 shadow-lg">
                        ...
                        <div className="flex flex-col justify-center">
                            <div className="text-[17px] font-bold leading-tight mb-1 tracking-tight drop-shadow-sm">
                                {memberInfo?.nickname || memberInfo?.name || lineProfile?.displayName || 'Member'}
                            </div>
                            <div className="flex items-center gap-2 opacity-100">
                                <span className="w-4 h-4 rounded-full bg-white flex items-center justify-center shadow-sm">
                                    <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: currentTier.textHex }}></span>
                                </span>
                                <span className="text-[13px] font-bold tracking-wide drop-shadow-sm">{currentTier.name}</span>
                            </div>
                            {activeTitle && (
                                <button 
                                  onClick={() => setShowCatalog(true)}
                                  className="mt-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wide flex items-center gap-1 shadow-sm active:scale-95 transition-transform self-start"
                            ...
                            {!activeTitle && (
                                <button onClick={() => setShowCatalog(true)} className="mt-1 text-[11px] font-medium text-white/80 hover:text-white transition-colors underline decoration-white/40 underline-offset-2 self-start">
                                    ดูฉายา
                                </button>
                            )}
                        </div>
                    ...
                    >
                        <div className="w-11 h-11 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-md shadow-lg border border-white/40 relative">
                           <Sparkles size={20} className="text-yellow-300 relative z-10 animate-pulse" />
                           <div className="absolute inset-0 rounded-full bg-white/30 animate-ping opacity-20" style={{ animationDuration: '2s' }}></div>
                        </div>
                        <span className="text-[9px] font-black uppercase tracking-[0.15em] opacity-90 drop-shadow-sm">สิทธิพิเศษ</span>
                    </button>
                </div>
                
                <div className="flex flex-col mt-2">
                  <span className="text-[12px] font-bold tracking-[0.25em] uppercase opacity-80 mb-1 drop-shadow-sm">
                    {dict.points}
                  </span>
                  <div className="flex items-baseline gap-2">
                    <span className="text-[64px] leading-none font-black tracking-tighter drop-shadow-md">
                      {(memberInfo?.points || 0).toLocaleString()}
                    </span>
                    <span className="text-[16px] font-bold opacity-80 uppercase tracking-widest drop-shadow-sm">Pts</span>
                  </div>
                </div>
            </div>
          </div>
