...
export default function MerchantStaffPage() {
  ...
  const fetchShopSettings = async () => {
    ...
    try {
      ...
      console.error('Error fetching merchant staff shop settings:', err)
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading || !shopSettings) {
    return <RUSHUPLoader tagline="กำลังเตรียมเครื่องมือจัดการพนักงาน..." />
  }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-3xl border border-zinc-200/60 shadow-sm">
        <div>
          ...
          <h1 className="text-xl sm:text-2xl font-black text-zinc-950 tracking-tight">
            จัดการบัญชีพนักงาน & สิทธิ์การใช้
          </h1>
          <p className="text-[11px] text-zinc-400 font-bold mt-0.5">
            กำหนดรหัส PIN พนักงานแคชเชียร์/คนรับออเดอร์ ตารางกะทำงาน และเวลาการเข้างานของสาขา
          </p>
        </div>
