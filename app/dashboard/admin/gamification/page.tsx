...
export default function AdminGamificationPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto font-sans">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Gamification Management</h1>
        <p className="text-gray-500 text-sm mt-1">Manage campaigns and mystery box rewards</p>
      </div>
      ...
      {activeTab === 'gacha' && (
        <>
          <div className="flex justify-between items-center mb-4">
            <div className="text-sm text-gray-500 bg-blue-50 text-blue-700 px-3 py-2 rounded-lg border border-blue-100">
              <strong>Tip:</strong> Ensure total Drop Rates equal 100% for predictable results. 
              Currently: {gachaPool.reduce((sum, item) => sum + (item.is_active ? Number(item.drop_rate_percentage) : 0), 0)}%
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    <th className="p-4 text-sm font-semibold text-gray-600">Reward Name</th>
                    <th className="p-4 text-sm font-semibold text-gray-600">Rarity</th>
                    <th className="p-4 text-sm font-semibold text-gray-600">Type / Value</th>
                    <th className="p-4 text-sm font-semibold text-gray-600">Drop Rate</th>
                    <th className="p-4 text-sm font-semibold text-gray-600">Status</th>
                    <th className="p-4 text-sm font-semibold text-gray-600 text-right">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {gachaLoading ? (
                    ...
                    <tr><td colSpan={6} className="p-8 text-center text-gray-500">No rewards configured.</td></tr>
                  ) : (
                    gachaPool.map((item) => (
                      <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="p-4">
                          <div className="text-sm font-medium text-gray-900 capitalize">{item.reward_type.replace('_', ' ')}</div>
                          {item.reward_type === 'points' && <div className="text-xs text-gray-500 mt-0.5">+{item.value_points} pts</div>}
                        </td>
                        <td className="p-4">
                          <div className="font-medium text-gray-900">{item.drop_rate_percentage}%</div>
                        </td>
                        <td className="p-4">
                          {item.is_active ? (
                            <span className="inline-flex items-center gap-1.5 py-1 px-2.5 rounded-full text-xs font-semibold bg-green-50 text-green-700 border border-green-200">
      ...
      {showGachaModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h2 className="text-lg font-bold text-gray-900">{editingGacha ? 'Edit Reward' : 'New Reward'}</h2>
            ...
            <form onSubmit={handleSaveGacha} className="p-6 overflow-y-auto">
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Reward Name <span className="text-red-500">*</span></label>
                  <input type="text" required value={gachaFormData.name} onChange={(e) => setGachaFormData({...gachaFormData, name: e.target.value})} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:border-black outline-none transition-shadow" placeholder="e.g., Free Iced Americano, +50 Points" />
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Rarity Tier <span className="text-red-500">*</span></label>
                    <select value={gachaFormData.rarity_tier} onChange={(e) => setGachaFormData({...gachaFormData, rarity_tier: e.target.value})} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:border-black outline-none bg-white">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Probability Weight <span className="text-red-500">*</span></label>
                    <input type="number" required min={0} value={gachaFormData.probability_weight} onChange={(e) => setGachaFormData({...gachaFormData, probability_weight: parseInt(e.target.value) || 0})} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:border-black outline-none" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea rows={2} value={gachaFormData.description} onChange={(e) => setGachaFormData({...gachaFormData, description: e.target.value})} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:border-black outline-none transition-shadow" placeholder="Internal notes or detailed description..." />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
                  <div>
                    ...
                    <select value={gachaFormData.is_active ? 'true' : 'false'} onChange={(e) => setGachaFormData({...gachaFormData, is_active: e.target.value === 'true'})} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:border-black outline-none bg-white">
                      <option value="true">Active (In Pool)</option>
                      <option value="false">Inactive</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Total Quantity Limit</label>
                    <input type="number" min={1} value={gachaFormData.max_quantity} onChange={(e) => setGachaFormData({...gachaFormData, max_quantity: e.target.value})} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:border-black outline-none" placeholder="Leave blank for unlimited" />
                  </div>
                </div>

              </div>
              <div className="mt-8 pt-4 border-t border-gray-100 flex gap-3">
                <button type="button" onClick={() => setShowGachaModal(false)} className="flex-1 px-4 py-3 border border-gray-200 text-gray-700 rounded-xl hover:bg-gray-50 transition-colors font-bold shadow-sm">Cancel</button>
                <button type="submit" className="flex-1 px-4 py-3 bg-[#1A1A18] text-white rounded-xl hover:bg-black transition-colors font-bold flex items-center justify-center gap-2 shadow-lg"><Save size={18} /> Save Reward</button>
              </div>
            </form>
          </div>
        </div>
