...
SELECT 'Bronze', 0, 1.0, 0, '#b45309', '["Base Earning Rate"]'::jsonb
WHERE NOT EXISTS (SELECT 1 FROM public.member_tiers WHERE name = 'Bronze');

INSERT INTO public.member_tiers (name, min_points, earn_multiplier, discount_percent, color_code, benefits)
SELECT 'Silver', 500, 1.2, 0, '#64748b', '["1.2x Earning Rate", "Free Birthday Drink Coupon"]'::jsonb
WHERE NOT EXISTS (SELECT 1 FROM public.member_tiers WHERE name = 'Silver');

INSERT INTO public.member_tiers (name, min_points, earn_multiplier, discount_percent, color_code, benefits)
SELECT 'Gold', 2000, 1.5, 5, '#ca8a04', '["1.5x Earning Rate", "5% Off All Orders", "Birthday Drink & Cake"]'::jsonb
WHERE NOT EXISTS (SELECT 1 FROM public.member_tiers WHERE name = 'Gold');

INSERT INTO public.member_tiers (name, min_points, earn_multiplier, discount_percent, color_code, benefits)
SELECT 'Platinum', 5000, 2.0, 10, '#0369a1', '["2x Earning Rate", "10% Off All Orders", "Free Annual Workshop"]'::jsonb
WHERE NOT EXISTS (SELECT 1 FROM public.member_tiers WHERE name = 'Platinum');
