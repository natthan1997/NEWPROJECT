...
const renderReceiptHtml = (order: PrintOrderData, shop: PrintShopData) => {
  ...
  if (order.loyaltyClaimQrUrl || order.loyaltyClaimToken) {
    ...
    const qrUrl = order.loyaltyClaimQrUrl || `https://api.qrserver.com/v1/create-qr-code/?size=240x240&data=${encodeURIComponent(`https://liff.line.me/${liffId}?claimToken=${token}&path=${encodeURIComponent(`/liff/member?claimToken=${token}`)}`)}`;
    html += `<div style="margin-top: 16px; border-top: 2px dashed #000; padding-top: 12px; text-align:center;">`;
