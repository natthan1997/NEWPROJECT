                  ...
                  if (respStep.tool_responses) {
                      for (const resp of respStep.tool_responses) {
                          if (resp.name === 'default_api:view_file' && resp.content) {
                              try {
                                  const c = JSON.parse(resp.content);
                                  if (c.output && c.output.includes('POSMenuManager.tsx')) {
                                       ...
                                       if (restoredLines.length > 650) {
                                            ...
                                            console.log('Restored POSMenuManager.tsx from good backup! Lines:', restoredLines.length);
                                            process.exit(0);
                                       }
                                  }
                              } catch(e) {}
...
console.log('Not found good backup!');
